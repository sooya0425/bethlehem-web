...
function SponsorshipDonationForm() {
  ...
  if (submitted) {
    return (
      ...
      >
        <div className="w-20 h-20 bg-accent/10 text-accent rounded-full flex items-center justify-center mx-auto mb-6 text-3xl">
          ✓
        </div>
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-bold mb-8 text-center">결연후원 신청서</h2>
      ...
      <div className="space-y-6">
        ...
        <div>
          <label className="block text-sm font-bold mb-2">매월 후원 금액</label>
          <input
            type="text"
            name="amount"
            value={form.amount}
        ...
        <div>
          <label className="block text-sm font-bold mb-2">원하시는 결연 대상 (선택)</label>
          <textarea
            name="message"
            value={form.message}
            onChange={handleChange}
            rows={4}
            placeholder="특별히 원하시는 결연 대상(연령대 등)이 있으시다면 자유롭게 적어주세요. 담당자가 상담 시 참고하겠습니다."
        ...
        <button
          ...
          className="w-full bg-accent text-white py-4 rounded-2xl font-bold text-lg hover:bg-accent/90 transition-all shadow-xl shadow-accent/20 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isSubmitting ? "전송 중..." : "결연후원 신청하기"}
        </button>
      </div>
    </motion.form>
  );
...
export default function SponsorshipDonationPage() {
  return (
    <main className="min-h-screen bg-background transition-colors duration-300">
      <Navbar />
      <SubMenuNav />
      
      <section className="pt-32 pb-20 bg-muted/80">
        <div className="container mx-auto px-6 text-center">
          ...
          >
            <h1 className="text-4xl md:text-5xl font-bold mb-6">결연후원 신청</h1>
            <p className="text-xl text-muted-foreground leading-relaxed">
              특정 장애인과 1:1 결연을 맺어 정서적 지지와 경제적 도움을 줍니다. <br/> 한 사람의 든든한 친구가 되어주세요.
            </p>
          </motion.div>
        </div>
      </section>
